...
-   **自定义着色器**：如果使用自定义Shader，需确保其与当前渲染管线兼容。`,"* * *"]},{heading:"总结",paragraphs:["拖入物体时，Unity完成了**资源处理→实例化→纳入渲染管线**的完整流程。渲染过程则是每帧按**剔除、几何处理、光照计算、像素着色、后期处理**的顺序执行，最终将物体视觉化。理解这一流程有助于优化美术资源和渲染性能。"]}],takeaways:[]}];export{n as D,e as a,t as p};
